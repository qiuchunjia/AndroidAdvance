package com.qcj.learning.view.exercise.gestureLockview;
 ...
 * mMarginBetweenLockView = mWidth ;
 * mGestureLockViewWidth=（mWidth-( n + 1 ) *mMarginBetweenLockView）/n
 * 得：mGestureLockViewWidth = 4 * mWidth / ( 5* mCount + 1 )
 * 注：mMarginBetweenLockView = mGestureLockViewWidth * 0.25 ;  （亲用笔算一下就清楚了哈）
 */
...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    public void initData(Context context, AttributeSet attrs) {
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs,
                R.styleable.GestureLockViewGroup, 0, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mGestureLockViews == null) {
            ...
            // 设置画笔的宽度为GestureLockView的内圆直径稍微小点（不喜欢的话，随便设） 内圆是0.3
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (child != null) {
                    ...
                    if (!mChoose.contains(cId)) {
                        mChoose.add(cId);
                        child.setMode(GestureLockView.Mode.STATUS_FINGER_ON); //设置它的模式，子view就会重绘
            ...
            case MotionEvent.ACTION_UP:
                ...
                for (int i = 0; i + 1 < mChoose.size(); i++) {
                    ...
                    // 计算角度
                    int angle = (int) Math.toDegrees(Math.atan2(dy, dx)) + 90; //比较关键的一步
                    startChild.setArrowDegree(angle);
                }
                break;

        }
        invalidate(); //每消费一个事件就重绘一次
        return true;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mPath!=null&&mPaint!=null){
            canvas.drawPath(mPath,mPaint);  //画手势线
        }
    }

    /**
     * 改变gesturelockview的模式
     */
