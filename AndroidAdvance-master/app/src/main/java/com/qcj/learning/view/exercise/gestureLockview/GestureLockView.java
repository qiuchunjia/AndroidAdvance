...
public class GestureLockView extends View {
     ...
     */
    private int mColorNoFingerInner;  // 手指要放上去的时候内部圆颜色
    private int mColorNoFingerOutter; //手指要放上去的时候外部圆颜色
    private int mColorFingerOn;//手指要放上去的时候颜色
    private int mColorFingerUp;//手指离开的时候的时候颜色
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mRadius -= mStrokeWidth / 2; //这里要减去笔的二分之一的宽度
    ...
    private void drawArrow(Canvas canvas) {
        if (mArrowDegree != -1) {
            mPaint.setStyle(Paint.Style.FILL);
            canvas.save(); //先保存画布已有的图像
            canvas.rotate(mArrowDegree, mCenterX, mCenterY); //然后旋转
            canvas.drawPath(mArrowPath, mPaint);
            canvas.restore();//画好后再还原
        }
    }
